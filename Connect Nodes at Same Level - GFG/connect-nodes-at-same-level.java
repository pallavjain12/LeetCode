...
class Solution {
    ...
    private static void fill(Node root, int index, ArrayList<ArrayList<Node>> lists) {
        if (root == null)   return;
        if (lists.size() == index) {
            lists.add(new ArrayList<Node>());
        }
        lists.get(index).add(root);
        fill(root.left, index + 1, lists);
        fill(root.right, index + 1, lists);
        return;
    }
}